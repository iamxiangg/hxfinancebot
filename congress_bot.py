...
async def send(items: list[str]) -> None:
    bot = Bot(token=TOKEN)
...
async def failure(text: str) -> None:
...
def main() -> None:
    ...
    try:
        ...
        if not scored:
            asyncio.run(failure("No ticker produced usable price analytics."))
            return
        ...
        asyncio.run(failure(str(exc)[:500]))
    finally:
        unlock()
